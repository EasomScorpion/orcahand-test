...
# 确保可以 import gesture_recognition 和 realtime_mirroring
THIS_DIR = os.path.dirname(os.path.abspath(__file__))
ROOT_DIR = os.path.dirname(THIS_DIR)  # FTServo_Python/
...
from realtime_mirroring.hand_tracker import HandTracker, SERVO_CONFIG
...
class TrackerWindow(QMainWindow):
    ...
    def _on_angles(self, angles):
        ...
        for i, sid in enumerate(range(1, 18)):
            ...
            cfg = SERVO_CONFIG[sid]
            jtype = cfg["type"]
            from realtime_mirroring.hand_tracker import ANGLE_RANGES, LATERAL_MAP, SERVO_ANGLE_RANGE
