...
3. **实时 viewer 联动真机**：拖 MuJoCo Control panel 滑条 → sim qpos → 17 舵机同步跟随（`orca_sim/live_control.py`）。

**范围**：
- orca_sim **v1** 右手 + `skin=False`（骨头对骨头碰撞，与真机一致）
- 真机 **STS3215 × 17**（右手 1..16 + wrist=17）
- FTServo_Python/ 侧**只追加**远端接收器（`RemotePoseReceiver` + `--remote-tcp-port` + 碰撞旁通按钮）；不动 SDK / `ServoSafetyLayer` / `ConsoleBackend` 的核心逻辑。
- orca_core/ 当前不在本项目范围内

**重要：方向翻转（per-servo）**：实测 sim 滑条正方向与真机舵机转动方向**大部分**相反，但也有少量关节（`right_index_abd / right_middle_abd / right_pinky_abd`）方向一致。bridge 在 `SimToRealDeployer._cache_per_servo` 里按 `ServoEntry.flip_direction`（默认 `true`）决定是否交换 `raw_low`/`raw_high` 存储语义（`true` 时 `sim_low → xdat.max_angle`、`sim_high → xdat.min_angle`），使 `qpos_to_positions` 输出已自带翻转。仍忽略零点偏移（CLAUDE.md §6.2 不变）。详见 [orca_sim/CLAUDE.md §13.1](orca_sim/CLAUDE.md)。
...
│   ├── CLAUDE.md                    # 详细的子项目说明
│   ├── pyproject.toml
│   ├── live_control.py              # ← 实时 viewer driver（顶层）
│   ├── src/orca_sim/
...
│   │       ├── deploy.py            # sim → 真机部署器（含方向翻转 + bypass latch）
│   │       ├── limits.py            # sim/xdat 对齐 advisory（含 detailed_alignment）
│   │       └── data/servo_joint_mapping.json
│   ├── scripts/sim_to_real.py       # ← batch CLI 入口（新增）
│   ├── scripts/calibrate_sim_to_real.py  # ← 手工校准工具（新增）
│   ├── tests/
│   │   ├── test_envs.py / test_versions.py / test_registry.py
│   │   ├── bridge/                  # ← 联动测试（新增）
│   │   └── test_live_control.py     # ← live_control 单元测试（fake 注入）
│   └── 其它 demo 脚本
...
**实时 viewer 联动**（拖滑条 → 真机同步跟随）：
```bash
# 终端 1：GUI 持有串口 + 起远端接收器
...
`live_control.py` 永远不碰串口；通过本地 TCP（`127.0.0.1:8765`）把每帧 raw 发给 GUI 的 `RemotePoseReceiver` 线程，由它走 `ServoSafetyLayer.sync_go_to_pose` 写硬件。GUI 控制 tab 有一个**「解除碰撞限制 (latch)」**按钮：按一次让 sim 端 `SimToRealDeployer.collision_bypass` 置位，下一帧若自碰撞则**照样下发**（单次放行），之后 latch 自动归零，再撞会再次被跳过。详细协议见 [orca_sim/README.md](orca_sim/README.md) 的「实时 sim→真机 控制」章节。
...
| 13 | 大拇指大指根 | `right_thumb_mcp` | mcp |
| 14 | 大拇指小指根 | `right_thumb_abd` | abd |
...
# === 手工校准 sim ↔ 真机 ===
python scripts/calibrate_sim_to_real.py inspect                    # 静态对齐检查
python scripts/calibrate_sim_to_real.py sample --output cal.csv    # 交互采样
python scripts/calibrate_sim_to_real.py fit --csv cal.csv           # 自动拟合 + flip 建议
...
2. **忽略零点的语义**：bridge 直接把 sim ctrlrange [low, high] ↔ xdat [min_angle, max_angle] 线性对应，**不补偿任何零点偏移**。若 sim ctrlrange 不对称（v1 `thumb_abd` 是 `[-1.08211, 0]`），真机舵机也会偏向 raw 范围的一侧——这是物理事实，不修复。方向翻转是叠加在「raw_low/raw_high 存储语义交换」层（见 §1），与本条互不影响。
3. **`<exclude>` 段**：v1 MJCF 已对相邻手指段 exclude，bridge 检测到的接触是「真正的」自碰撞（不是误判）。
4. **FTServo 串口独占**：orca_sim bridge 与 `servo_console.py` GUI 不能同时跑（pyserial 独占打开）。`live_control.py` 永远不碰串口，TCP 走 localhost。
...
9. **碰撞旁通 latch 语义**：`SimToRealDeployer.collision_bypass` 是**单次放行** latch —— GUI 按一下后只有下一帧的碰撞被照样下发，latch 自动归零。如果想连续多次放行，每帧都需要重新按按钮。**不要**把它当成 "永久禁用碰撞检查" 用——碰撞是真机可能撞的信号。
...
cd orca_sim
source orca/Scripts/activate
python -m pytest tests/ tests/bridge/ tests/test_live_control.py -q --ignore=tests/retarget
```

**原 orca_sim 测试**（tests/）共 35 个，已在 v0.1.0 通过。
**bridge 测试**（tests/bridge/）共 26 个：

- `test_mapping.py` — JSON 加载、servo_id ↔ sim_actuator 双向查表、缺失/越界报错
- `test_deploy.py` — 弧度→raw 转换（含**方向翻转**后的 clip 行为）、ServoSafetyLayer 校验路径、单调性、忽略零点
- `test_collision.py` — CollisionGuard 自碰撞过滤、threshold 边界、过滤背景 geom
- `test_limits.py` — sim ctrlrange 与 xdat 范围对齐检查（advisory）

**live_control 测试**（tests/test_live_control.py）共 20 个：

- JSON wire 格式 + `PoseTcpServer`（host/port 校验、完整 round-trip、无 client 时 broadcast、**反向命令分发 + 垃圾行容错**）
- `run_live_loop`（碰撞跳过、dry-run、断开帧、`max_frames` 停机、render/step 顺序、**碰撞旁通 latch 单次消费**）
- `KeyboardInterrupt` 优雅退出；与真实 `OrcaHandRight` env 的 rad→raw 转换一致性
...
- orca_sim 原 35 个测试 + bridge 26 个 + live_control 20 个 = **81 / 81 passed**（已排除无关的 `tests/retarget`）。
- `python live_control.py --no-render --max-fps 5` dry-run 正常（每帧打印 17 路 raw）。
- `python live_control.py --port COM5 --max-fps 5` 起 `127.0.0.1:8765` TCP server 正常（待 GUI receiver）。
...
`FT/` = 真实手控制程序 (`FTServo_Python/`) + 仿真环境 (`orca_sim/`) + 联动桥 (`orca_sim/src/orca_sim/bridge/`) + 实时 viewer driver (`orca_sim/live_control.py`)。

**核心思路**：用 sim 自带的 STL 碰撞检测过滤「动作可执行性」；sim ctrlrange ↔ xdat raw 线性映射（**含方向翻转** + 忽略零点）；通过 `ServoSafetyLayer.sync_go_to_pose` 原子下发 17 个 STS3215 舵机；GUI 与 sim 通过 `127.0.0.1:<port>` 本地 TCP 通信（pyserial 独占），**碰撞旁通 latch** 让用户可以手动放行一次性过冲。