...
#
# min/max 来自 参数/{servo_id}.xdat 的「最小角度限制」/「最大角度限制」，
# 必须与 xdat 保持一致，否则 angles_to_positions 会产出超出固件限位的
# 位置，导致 sync_go_to_pose 抛出 SafetyViolation（机械手在极限姿态卡死）。
# ---------------------------------------------------------------------------
SERVO_CONFIG: dict[int, dict] = {
    1:  {"min": 1385, "max": 2468, "type": "tip",    "finger": "index",  "invert": False, "name": "食指指尖"},
    2:  {"min": 1500, "max": 2520, "type": "mid",    "finger": "index",  "invert": False, "name": "食指中节"},
    3:  {"min": 1790, "max": 2400, "type": "bottom", "finger": "index",  "invert": False, "name": "食指根部"},
    4:  {"min": 1790, "max": 2400, "type": "bottom", "finger": "middle", "invert": False, "name": "中指根部"},
    5:  {"min": 1723, "max": 2270, "type": "bottom", "finger": "pinky",  "invert": False, "name": "小指根部"},
    ...
    8:  {"min": 1695, "max": 2760, "type": "tip",    "finger": "middle", "invert": True,  "name": "中指指尖"},
    9:  {"min": 1470, "max": 2536, "type": "mid",    "finger": "middle", "invert": False, "name": "中指中节"},
    10: {"min": 1640, "max": 2570, "type": "mid",    "finger": "ring",   "invert": False, "name": "无名指中节"},
    11: {"min": 1375, "max": 2450, "type": "tip",    "finger": "ring",   "invert": False, "name": "无名指指尖"},
    12: {"min": 1720, "max": 2330, "type": "bottom", "finger": "ring",   "invert": True,  "name": "无名指根部"},
    13: {"min": 1340, "max": 2230, "type": "thumb_big",     "finger": "thumb",  "invert": False, "name": "拇指大根部"},
    14: {"min": 1615, "max": 2120, "type": "thumb_lateral", "finger": "thumb",  "invert": True, "name": "拇指小根部"},
    15: {"min": 1500, "max": 2530, "type": "thumb_tip",     "finger": "thumb",  "invert": False, "name": "拇指指尖"},
    16: {"min": 1309, "max": 2380, "type": "thumb_mid",     "finger": "thumb",  "invert": False, "name": "拇指中节"},
    17: {"min": 1300, "max": 2500, "type": "wrist",  "finger": "wrist",  "invert": True,  "name": "手腕"},
}
...
LATERAL_MAP: dict[int, dict] = {
    ...
    5:  {"A": {"mid_angle": -3.0, "neg_range": 5.0, "pos_range": 24.0,
               "min_pos": 1790, "max_pos": 2270},
         "B": {"pos_range": 7.0, "min_pos": 1790, "max_pos": 2270}},
        ...
        for sid, cfg in SERVO_CONFIG.items():
            ...
            if sid in angles:
                ...
                if jtype in ("bottom", "thumb_lateral") and sid in LATERAL_MAP:
                    ...
                    pos = max(cfg["min"] + 50, min(cfg["max"] - cfg.get("max_offset", 50), pos))
                    positions[sid] = pos
