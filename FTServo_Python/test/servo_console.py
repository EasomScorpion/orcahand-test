...
sys.path.insert(0, ROOT_DIR)     # 用于 import xdat_tool
sys.path.insert(0, "..")          # 用于 import scservo_sdk

from scservo_sdk import *          # noqa: E402
import xdat_tool                   # noqa: E402

from PyQt5.QtCore import Qt, QObject, pyqtSignal, QAbstractTableModel, QModelIndex  # noqa: E402
from PyQt5.QtWidgets import (                                            # noqa: E402
    ...
    QGroupBox, QSizePolicy, QInputDialog, QCheckBox,
...
class SafetyLimits:
    """硬底线安全参数。任何功能都不能修改这些值。"""
    MAX_TORQUE:      int = 100
    PROTECT_TORQUE:  int = 20
    OVERLOAD_TORQUE: int = 20
    PROTECT_TIME:    int = 20
    # 复位动作（出厂保守默认）
...
class RPSGameEngine(QObject):
    ...
    def _run(self):
        ...
        try:
            ...
            from gesture_recognition.gesture_rps import RPSClassifier
    ...
    def _wait_and_clear_busy(self):
        """子线程：轮询 ReadMoving 等机械手完成，然后清忙标志。"""
    ...
    def _execute_winning(self, user_label):
        ...
        try:
            self._backend.safety.sync_go_to_pose(pose.positions)
...
class ServoSafetyLayer:
    ...
    # ─── 安全底线（启动时写一次） ─────────────────────────────────────
    def apply_safety_floor(self, scs_id):
        """对单个舵机写入硬底线安全参数。"""
        self._ph.unLockEprom(scs_id)
        try:
            self._ph.write2ByteTxRx(scs_id, SMS_STS_MAX_TORQUE_L,
                                    self._limits.MAX_TORQUE)
            self._ph.write1ByteTxRx(scs_id, SMS_STS_PROTECT_TORQUE,
                                    self._limits.PROTECT_TORQUE)
            self._ph.write1ByteTxRx(scs_id, SMS_STS_OVERLOAD_TORQUE,
                                    self._limits.OVERLOAD_TORQUE)
            self._ph.write1ByteTxRx(scs_id, SMS_STS_PROTECT_TIME,
                                    self._limits.PROTECT_TIME)
        finally:
    ...
    def write_xdat_to_firmware(self, scs_id):
        ...
        最小角度限制, 最大角度限制。
        所有值均来自该舵机的 xdat 参数文件 (bundle.fields)。
        """
...
# ===========================================================================
# 远端姿态接收器：监听 orca_sim/live_control.py 推送的 17 路舵机目标
# ===========================================================================
class RemotePoseReceiver(QObject):
    """后台线程：连接到 ``orca_sim.live_control.PoseTcpServer``，把每帧
    ``{servo_id: raw}`` 转交给 :class:`ServoSafetyLayer.sync_go_to_pose`。

    启动方式::

        receiver = RemotePoseReceiver(backend, host="127.0.0.1", port=8765)
        receiver.start()       # 启动 daemon 连接循环
        receiver.stop()        # 关闭 socket + 退出线程

    线程安全：
        所有 Qt 控件更新请通过 :attr:`status_changed` / :attr:`error` 信号
        转发到主线程；不要在本线程直接 ``QLabel.setText``。

    串口未连：
        :meth:`_handle_line` 检查 ``backend.connected and backend.safety``，
        未连时打日志并跳过，不抛异常、不退出线程。
    """

    status_changed = pyqtSignal(bool, str)  # (connected, message)
    error = pyqtSignal(str)                # human-readable error
    # 反方向：sim → GUI 发来的非 position payload（如 apply_limits）。
    # GUI 主线程连接后处理 EPROM 写入等副作用。
    payload_received = pyqtSignal(dict)

    def __init__(self, backend, host="127.0.0.1", port=8765,
                 reconnect_delay=1.0):
        super().__init__()
        self._backend = backend
        self.host = host
        ...
        self._thread = None
        self._socket = None  # 当前持有 socket（用于 stop 时关掉）
    ...
    def start(self):
        ...
        self._stop.clear()
        self._thread = threading.Thread(
            target=self._run, name="remote-pose-receiver", daemon=True
        )
        self._thread.start()
    ...
    def stop(self, timeout=2.0):
        """停止线程 + 关闭 socket。"""
        self._stop.set()
        # 关闭当前 socket 以唤醒阻塞的 recv/connect
    ...
    # ─── 反向通道（GUI → live_control.py） ────────────────────────
    def send_command(self, payload):
        """在当前 socket 上发一条 newline-delimited JSON 命令给 live_control.py。

        TCP 是全双工的：发送不会影响接收线程；但**必须等连接建立后再发**，
        所以这里在 ``self.connected`` 为 True 时才写。
        失败（未连 / socket 已关 / OSError）不抛、不影响接收循环。
        """
    ...
    def request_bypass_collision(self):
        """便捷方法：发 ``{"type": "bypass_collision"}`` 给 live_control.py。"""
        return self.send_command({"type": "bypass_collision"})

    def request_reload_xdat(self):
        """便捷方法：发 ``{"type": "reload_xdat"}`` 给 live_control.py。

        sim 端收到后会重新读取 参数/*.xdat 并刷新 SimToRealDeployer._info
        里的 raw_high/raw_low 缓存，下一帧即可生效；不改 EPROM、不改 xdat。
        """
        return self.send_command({"type": "reload_xdat"})

    # ─── 内部 ───────────────────────────────────────────────────────

    def _set_connected(self, value, msg):
        if self.connected != value:
            self.connected = value
            try:
                self.status_changed.emit(value, msg)
            except Exception:
                pass
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                if self._stop.is_set():
                    break
                # 1s 后重连
                self._set_connected(False, f"等待 {self.host}:{self.port} …")
                sock.close()
                self._stop.wait(self._reconnect_delay)
                continue

            self._socket = sock
            self._set_connected(True, f"已连 {self.host}:{self.port}")
            try:
                fp = sock.makefile("rb")
                buf = b""
                while not self._stop.is_set():
                    ...
                    while b"\n" in buf:
                        ...
                        if line.strip():
                            try:
                                ...
                                self._emit_error(f"处理姿态失败: {exc}")
            finally:
                self._set_connected(False, "连接已断开")
                try:
                    sock.close()
                except Exception:
                    pass
    ...
    def _handle_line(self, line: bytes):
        """解析 ``{"positions": {"1": 2048, ...}}`` 并下发到 safety。"""
        try:
            payload = json.loads(line.decode("utf-8"))
        except (ValueError, UnicodeDecodeError) as exc:
            self._emit_error(f"远端帧解析失败: {exc}")
            return
        if not isinstance(payload, dict):
            self._emit_error("payload 不是 dict")
            return
        # 反方向 payload（sim → GUI 推的非 positions 消息，如 apply_limits）
        if "positions" not in payload:
            self._emit_error(f"未知 payload type: {payload.get('type', '?')}")
            try:
        ...
        for sid, raw in raw_positions.items():
            try:
                positions[int(sid)] = int(raw)
            except (TypeError, ValueError):
                continue
        if not positions:
            return
        # 串口未连 → 跳过本帧
        if not self._backend.connected or self._backend.safety is None:
            self._emit_error("串口未连接，远端姿态被跳过")
            return
        try:
            self._backend.safety.sync_go_to_pose(positions)
        except SafetyViolation as exc:
            self._emit_error(f"安全越界: {exc}")
        except Exception as exc:  # noqa: BLE001
            self._emit_error(f"sync_go_to_pose 失败: {exc}")
...
class ConsoleBackend(QObject):
    ...
    def __init__(self):
        ...
        # xdat 自动持久化：写入 EPROM 后是否同步写回 .xdat 文件
        self.auto_save_xdat = True
    ...
    # ─── xdat 自动持久化 ──────────────────────────────────────────────
    def save_bundle_xdat(self, scs_id):
        """把单个舵机的内存字段写回 参数/{scs_id}.xdat 文件。

        任何修改 bundle.fields 的代码都应在修改后调用本方法，
        这样断电/重启后修改仍然存在。
        """
        bundle = next((b for b in self._bundles if b.servo_id == scs_id), None)
        if bundle is None:
            return False
        path = os.path.join(PARAM_DIR, f"{scs_id}.xdat")
        try:
            xdat_tool.write_xdat(path, bundle.fields)
            return True
        except Exception as exc:
            print(f"⚠ 保存 {path} 失败: {exc}", file=sys.stderr)
            return False
...
class ConsoleWindow(QMainWindow):
    ...
    def __init__(self, remote_tcp_port=None):
        super().__init__()
        ...
        # 远端姿态接收器（--remote-tcp-port 启用）
        self._remote_receiver = None
        if remote_tcp_port is not None:
            self._remote_receiver = RemotePoseReceiver(
                self._backend, host="127.0.0.1", port=int(remote_tcp_port),
            )
        ...
        # xdat 加载完毕后再启动 receiver（safety limits 已就位）
        if self._remote_receiver is not None:
            self._remote_receiver.start()
    ...
    def _build_control_tab(self):
        ...
        # ─ 远端控制 (live_control.py) ─
        # 仅当 --remote-tcp-port 启动时显示；默认隐藏。
        self.remote_status_label = QLabel("远端控制: 未启用")
        self.remote_status_label.setStyleSheet(
            "padding: 4px; font-weight: bold; "
            "background-color: #9E9E9E; color: white;"
        )
        self.remote_status_label.setVisible(self._remote_receiver is not None)
        layout.addWidget(self.remote_status_label)

        # ─ 解除碰撞限制（一次性 latch） ─
        # 只有连上远端 live_control.py 才显示；按一下发反向命令让 sim 端
        # 放行下一次自碰撞帧；之后 latch 自动归零、再撞照旧被跳过。
        self.bypass_btn = QPushButton("解除碰撞限制 (latch)")
        self.bypass_btn.setStyleSheet(
            "background-color: #FF9800; color: white; "
            "font-weight: bold; padding: 6px;"
        )
        self.bypass_btn.setToolTip(
            "按一次后，下一帧若 sim 检测到自碰撞则照常下发（不跳过）。\n"
            "若下一帧仍碰撞，会再次被跳过——单次放行。"
        )
        self.bypass_btn.setVisible(self._remote_receiver is not None)
        self.bypass_btn.clicked.connect(self._on_bypass_clicked)
        layout.addWidget(self.bypass_btn)

        # ─ 安全底线 + 复位 ─
        action_layout = QHBoxLayout()
        self.apply_safety_btn = QPushButton("应用安全底线 (MAX=100, 保护=20)")
        self.apply_safety_btn.setStyleSheet(
            "background-color: #2196F3; color: white; "
            "font-weight: bold; padding: 10px;"
        )
        self.apply_limits_from_sim_btn = QPushButton("📥 应用 sim 限位到 EPROM (17 路)")
        self.apply_limits_from_sim_btn.setStyleSheet(
            "background-color: #9C27B0; color: white; "
            "font-weight: bold; padding: 10px;"
        )
        self.apply_limits_from_sim_btn.setToolTip(
            "向 orca_sim/live_control.py 拉取当前 17 路 sim 姿态→raw 目标，"
            "作为 EPROM 寄存器 9/11（min/max 角度）的写入建议。\n"
            "需要 live_control 用 --port COMx --calibrate-limits 启动。"
        )
        self.apply_limits_from_sim_btn.setVisible(self._remote_receiver is not None)
        self.reset_btn = QPushButton("复位到 2048 (原点)")
        self.reset_btn.setStyleSheet(
            "background-color: #4CAF50; color: white; "
            "font-weight: bold; padding: 10px;"
        )
        action_layout.addWidget(self.apply_safety_btn)
        action_layout.addWidget(self.apply_limits_from_sim_btn)
        action_layout.addWidget(self.reset_btn)
        layout.addLayout(action_layout)

        # ─ xdat 批量写固件 ─
        xdat_layout = QHBoxLayout()
        self.write_xdat_btn = QPushButton("📥 写入 xdat 参数到全部 17 舵机")
        self.write_xdat_btn.setStyleSheet(
            "background-color: #673AB7; color: white; "
            "font-weight: bold; padding: 10px;"
        )
        self.write_xdat_btn.setToolTip(
            "把 参数/*.xdat 文件里的字段（最大扭矩 / 保护扭矩 / 过载扭矩 / "
            "保护时间 / 最小角度限制 / 最大角度限制）写入全部 17 个舵机的 EEPROM。\n"
            "⚠ 断电后仍然有效。请确认 xdat 文件内容正确后再点。"
        )
        self.reload_xdat_btn = QPushButton("🔄 重新加载 xdat 映射")
        self.reload_xdat_btn.setStyleSheet(
            "background-color: #009688; color: white; "
            "font-weight: bold; padding: 10px;"
        )
        self.reload_xdat_btn.setToolTip(
            "向 live_control.py 发 reload_xdat，让 sim 端重新读取 参数/*.xdat "
            "并刷新 raw_high/raw_low 缓存。\n"
            "改完 EPROM 限位后按一下，拖滑条立刻反映新范围，无需重启 sim。\n"
            "需要 live_control 用 --port COMx 启动。"
        )
        self.reload_xdat_btn.setVisible(self._remote_receiver is not None)
        self.auto_save_checkbox = QCheckBox("写入后自动保存到 .xdat")
        self.auto_save_checkbox.setChecked(True)
        self.auto_save_checkbox.setToolTip(
            "启用后，每次写入 EPROM 寄存器时同时把新值写回 参数/N.xdat 文件，\n"
            "保证下次启动时 xdat 与舵机固件一致。"
        )
        xdat_layout.addWidget(self.write_xdat_btn)
        xdat_layout.addWidget(self.reload_xdat_btn)
        xdat_layout.addWidget(self.auto_save_checkbox)
        xdat_layout.addStretch()
        layout.addLayout(xdat_layout)
        ...
        self.write_one_xdat_btn.setToolTip(
            "把当前舵机的 参数/N.xdat 字段写入 EEPROM:\n"
            "最大扭矩 / 保护扭矩 / 过载扭矩 / 保护时间 / 最小/最大角度限制。"
        )
        self.write_one_xdat_btn.setStyleSheet(
            "background-color: #673AB7; color: white; "
            "font-weight: bold; padding: 4px 10px;"
        )
        limit_row.addWidget(self.write_one_xdat_btn)
        ...
        self.apply_safety_btn.clicked.connect(self._on_apply_safety)
        self.apply_limits_from_sim_btn.clicked.connect(self._on_apply_limits_from_sim)
        self.reset_btn.clicked.connect(self._on_reset)
        self.write_xdat_btn.clicked.connect(self._on_write_xdat_to_firmware)
        self.write_one_xdat_btn.clicked.connect(self._on_write_one_xdat_to_firmware)
        self.reload_xdat_btn.clicked.connect(self._on_reload_xdat_clicked)
    ...
    def _wire_remote(self):
        """绑定 RemotePoseReceiver 的信号到 UI 状态。"""
        self._remote_receiver.status_changed.connect(self._on_remote_status)
        self._remote_receiver.error.connect(self._on_remote_error)
        self._remote_receiver.payload_received.connect(self._on_apply_limits_payload)

    def _on_remote_status(self, connected: bool, message: str):
        """UI 线程槽：更新状态标签 + 状态栏。"""
        if not hasattr(self, "remote_status_label"):
            return
        if connected:
            self.remote_status_label.setText(f"远端控制: 已连接 — {message}")
            self.remote_status_label.setStyleSheet(
                "padding: 4px; font-weight: bold; "
                "background-color: #4CAF50; color: white;"
            )
        else:
            self.remote_status_label.setText(f"远端控制: 未连接 — {message}")
            self.remote_status_label.setStyleSheet(
                "padding: 4px; font-weight: bold; "
                "background-color: #F44336; color: white;"
            )
        self._status(f"远端 {message}")

    def _on_remote_error(self, msg: str):
        """UI 线程槽：远端接收出错时打到状态栏。"""
        self._status(f"[remote] {msg}")

    def _on_bypass_clicked(self):
        """UI 线程槽：按下「解除碰撞限制」按钮 → 发反向命令给 sim 端。"""
        if not hasattr(self, "_remote_receiver") or self._remote_receiver is None:
            QMessageBox.information(self, "提示", "未启用远端控制（需用 --remote-tcp-port 启动）。")
            return
        if not self._remote_receiver.connected:
            QMessageBox.information(self, "提示", "还没连上 live_control.py；先等连接灯变绿再按。")
            return
        ok = self._remote_receiver.request_bypass_collision()
        if ok:
            self._status("[remote] 已请求单次碰撞放行")
        else:
            self._status("[remote] 请求失败（socket 已断开？）")

    def _on_reload_xdat_clicked(self):
        """UI 线程槽：按下「🔄 重新加载 xdat 映射」按钮 → 发反向命令给 sim 端
        让 deployer 重新读取 xdat 刷新 _info 缓存（不改 EPROM、不改 xdat 文件）。"""
        if not hasattr(self, "_remote_receiver") or self._remote_receiver is None:
            QMessageBox.information(
                self, "提示", "未启用远端控制（需用 --remote-tcp-port 启动）。",
            )
            return
        if not self._remote_receiver.connected:
            QMessageBox.information(
                self, "提示", "还没连上 live_control.py；先等连接灯变绿再按。",
            )
            return
        ok = self._remote_receiver.request_reload_xdat()
        if ok:
            self._status("[remote] 已请求 sim 重新加载 xdat 映射缓存")
        else:
            self._status("[remote] 请求失败（socket 已断开？）")
    ...
    def _on_apply_safety(self):
        ...
        try:
            ...
            for b in self._backend.bundles:
                try:
                    self._backend.safety.apply_safety_floor(b.servo_id)
                except Exception as e:
            ...
            if errors:
                QMessageBox.warning(self, "部分失败",
                                    "以下舵机写入失败:\n\n" +
                                    "\n".join(errors))
                self._status(f"⚠ 安全底线部分应用: {len(errors)} 个失败")
            else:
                self._status(
                    "✅ 安全底线已全部应用 "
                    "(MAX=100, PROTECT=20, OVERLOAD=20, PROTECT_TIME=20)"
                )
        finally:
    ...
    def _on_servo_changed(self, idx):
        ...
        self.limit_label.setText(
            f"当前角度限制: min={bundle.min_angle}  "
            f"max={bundle.max_angle}  (来自 {scs_id}.xdat)"
        )
        self.offset_label.setText(f"当前偏移: {bundle.ofs}")
        # 同步 min/max 编辑框
        self.min_angle_spin.blockSignals(True)
        self.min_angle_spin.setValue(bundle.min_angle)
        self.min_angle_spin.blockSignals(False)
        self.max_angle_spin.blockSignals(True)
        self.max_angle_spin.setValue(bundle.max_angle)
        ...
        if self._backend.connected:
            ...
            if comm == COMM_SUCCESS:
                ...
                init_pos = (bundle.min_angle + bundle.max_angle) // 2
        ...
        self.slider.setRange(bundle.min_angle, bundle.max_angle)
    ...
    def _on_set_mid_position(self):
        ...
        try:
            ...
            xdat_note = f"，已写入 {scs_id}.xdat" if saved else "，⚠ xdat 保存失败"
            self._status(
                f"✅ ID {scs_id} 中位校准完成: "
                f"位置偏移 {old_offset} → {new_offset}, "
                f"物理 {pos} → 逻辑 2048{xdat_note}"
            )
        except SafetyViolation as e:
    ...
    # ─── 从 sim 拉限位建议 → 写 EPROM（人工校验工作流） ────────────────
    def _on_apply_limits_from_sim(self):
        """向 live_control.py 发 ``request_limits``，等回 ``apply_limits``，
        然后逐 servo 弹窗确认 → 写 EPROM 寄存器 9/11。
        """
        if not self._backend.connected or not self._backend.safety:
            QMessageBox.warning(self, "未连接", "请先连接串口")
            return
        if self._remote_receiver is None:
            QMessageBox.warning(
                self, "未启用远端",
                "需要 live_control.py 用 --port 启动（已带 --remote-tcp-port）"
            )
            return
        # 走 receiver 的 send_command 发 request_limits
        if not self._remote_receiver.send_command({"type": "request_limits"}):
            QMessageBox.warning(
                self, "发送失败",
                "远端 sim 未连接；检查 live_control.py 是否在跑 "
                "(--port COMx --calibrate-limits)"
            )
            return
        # receiver 的反方向收 + 走 on_apply_limits_received 是另一条路径
        # （receiver 收到 apply_limits 时调 self._on_apply_limits_payload）。
        # 这里只是发个请求 + 给用户即时反馈；写 EPROM 实际由 callback 触发。
        self._status("⏳ 已向 sim 发 request_limits，等待 apply_limits payload...")

    def _on_apply_limits_payload(self, payload):
        """``RemotePoseReceiver`` 收到 ``{"type":"apply_limits","limits":{...}}`` 时调用。

        逐 servo 弹窗确认 → 写 EPROM 寄存器 9/11 → 读回验证 → 更新内存。
        """
        if not isinstance(payload, dict):
            logger.warning("apply_limits payload 不是 dict: %r", payload)
            return
        limits = payload.get("limits", {})
        if not isinstance(limits, dict) or len(limits) != 17:
            QMessageBox.warning(
                self, "payload 非法",
                f"limits 必须恰好 17 个 servo；收到 {len(limits) if isinstance(limits, dict) else '?'}",
            )
            return

        # 解析 + 校验每条
        parsed_limits: dict[int, tuple[int, int]] = {}
        try:
            for sid_str, val in limits.items():
                sid = int(sid_str)
                if not (1 <= sid <= 17):
                    raise ValueError(f"sid={sid} 超出 1..17")
                if not isinstance(val, dict):
                    raise ValueError(f"sid={sid}: val 不是 dict")
                lo = int(val["min"])
                hi = int(val["max"])
                if not (0 <= lo <= 4095) or not (0 <= hi <= 4095):
                    raise ValueError(f"sid={sid}: raw 越界 [{lo},{hi}]")
                if lo >= hi:
                    raise ValueError(f"sid={sid}: min({lo}) >= max({hi})")
                parsed_limits[sid] = (lo, hi)
        except (KeyError, TypeError, ValueError) as exc:
            QMessageBox.warning(self, "payload 校验失败", str(exc))
            return

        # 显示汇总 + 让用户先总览（一个汇总弹窗，不弹 17 次以减少噪音）
        chinese_by_sid = {b.servo_id: b.fields.get("中文名", f"ID{b.servo_id}")
                          for b in self._backend.bundles}
        summary_lines = []
        for sid in sorted(parsed_limits):
            lo, hi = parsed_limits[sid]
            summary_lines.append(
                f"  ID {sid:>2} ({chinese_by_sid.get(sid, '?')}): "
                f"min={lo}  max={hi}"
            )
        ok = QMessageBox.question(
            self, "应用 sim 限位到 EPROM",
            "将向 EPROM 寄存器 9（min）/ 11（max）写入以下 17 路限位：\n\n"
            + "\n".join(summary_lines) +
            "\n\n⚠ EPROM 写入不可逆。\n"
            "建议先确认 sim 滑条已拖到真机机械极限。\n\n确定继续？",
            QMessageBox.Yes | QMessageBox.No,
        )
        if ok != QMessageBox.Yes:
            self._status("⏸ 用户取消应用 sim 限位")
            return

        # 逐 servo 写 EPROM + 读回验证
        QApplication.setOverrideCursor(Qt.WaitCursor)
        errors: list[str] = []
        applied: list[tuple[int, int, int]] = []  # (sid, written_min, written_max)
        try:
            for sid in sorted(parsed_limits):
                lo, hi = parsed_limits[sid]
                try:
                    self._backend.safety.write_eprom_register(sid, 9, 2, lo)
                    self._backend.safety.write_eprom_register(sid, 11, 2, hi)
                except SafetyViolation as exc:
                    errors.append(f"ID {sid}: 安全越界: {exc}")
                    continue
                except Exception as exc:  # noqa: BLE001
                    errors.append(f"ID {sid}: 写入失败: {exc}")
                    continue
                # 读回验证（如果 protocol handler 提供 read2ByteTxRx）
                try:
                    actual_lo = int(self._backend.safety._ph.read2ByteTxRx(sid, 9))
                    actual_hi = int(self._backend.safety._ph.read2ByteTxRx(sid, 11))
                    if actual_lo != lo or actual_hi != hi:
                        errors.append(
                            f"ID {sid}: 写入 {lo}/{hi} 但读回 {actual_lo}/{actual_hi}"
                        )
                        continue
                except Exception as exc:  # noqa: BLE001
                    # 读回失败不阻塞写入结果（仅记 warning）
                    self._status(f"⚠ ID {sid} 读回 EPROM 失败: {exc}")
                # 写成功 → 更新内存 bundle + UI SpinBox
                bundle = next(
                    (b for b in self._backend.bundles if b.servo_id == sid), None
                )
                if bundle is not None:
                    bundle.min_angle = lo
                    bundle.max_angle = hi
                    bundle.fields["最小角度限制"] = lo
                    bundle.fields["最大角度限制"] = hi
                    # 自动写回 .xdat
                    self._save_xdat(sid)
                applied.append((sid, lo, hi))
        finally:
            QApplication.restoreOverrideCursor()

        # 刷新 UI（如果当前选中的是刚写入的 servo，更新 SpinBox）
        cur_sid = self.servo_combo.currentData()
        if cur_sid is not None and cur_sid in parsed_limits:
            self.min_angle_spin.blockSignals(True)
            self.max_angle_spin.blockSignals(True)
            self.min_angle_spin.setValue(parsed_limits[cur_sid][0])
            self.max_angle_spin.setValue(parsed_limits[cur_sid][1])
            self.min_angle_spin.blockSignals(False)
            self.max_angle_spin.blockSignals(False)
            self.slider.setRange(parsed_limits[cur_sid][0], parsed_limits[cur_sid][1])
            self.limit_label.setText(
                f"当前角度限制: min={parsed_limits[cur_sid][0]}  "
                f"max={parsed_limits[cur_sid][1]}  (已从 sim 应用)"
            )

        if errors:
            QMessageBox.warning(
                self, "部分失败",
                f"{len(applied)}/17 写入成功，{len(errors)} 个失败：\n\n"
                + "\n".join(errors[:10])
                + ("\n..." if len(errors) > 10 else ""),
            )
            self._status(
                f"⚠ sim 限位部分应用: {len(applied)} ok, {len(errors)} failed"
            )
        else:
            self._status(
                f"✅ sim 限位全部应用 ({len(applied)}/17)，EPROM 寄存器 9/11 已更新"
            )
    ...
    def _on_apply_limits(self):
        ...
        try:
            ...
            self._status(
                f"✅ ID {scs_id} 角度限制已更新: "
                f"min={new_min}, max={new_max}{xdat_note}"
            )
        except SafetyViolation as e:
            QMessageBox.warning(self, "急停中", str(e))
        except Exception as e:
    ...
    # ─── 写入 xdat 参数到舵机固件 ──────────────────────────────────────
    def _save_xdat(self, scs_id):
        """根据 auto_save_checkbox 状态决定是否写回 .xdat 文件。返回 True 表示已写。"""
        if not self.auto_save_checkbox.isChecked():
            return False
        return self._backend.save_bundle_xdat(scs_id)

    def _on_auto_save_changed(self, state):
        self._backend.auto_save_xdat = bool(state)
        self._status(
            f"📁 xdat 自动保存: {'开启' if state else '关闭'}"
        )

    def _on_write_xdat_to_firmware(self):
        """把 参数/*.xdat 字段写入全部 17 个舵机 EEPROM。"""
        if not self._backend.connected:
            QMessageBox.warning(self, "未连接", "请先连接串口再写入固件参数。")
            return
        if not self._backend.bundles:
            QMessageBox.warning(self, "无 xdat", "未加载任何 xdat 参数。")
            return
        ...
        try:
            ...
            if errors:
                ...
                self._status("✅ 已将 xdat 参数写入全部 17 个舵机固件")
        finally:
            QApplication.restoreOverrideCursor()

    def _on_write_one_xdat_to_firmware(self):
        """把当前选中舵机的 xdat 字段写入其 EEPROM。"""
        if not self._backend.connected:
            QMessageBox.warning(self, "未连接", "请先连接串口再写入固件参数。")
            return
        idx = self.servo_combo.currentIndex()
        ...
        try:
            ...
            self._status(f"✅ 已将 xdat 参数写入舵机 {scs_id} 固件")
            # 刷新 UI（重读 EPROM 并更新 spinbox）
            self._on_servo_changed(idx)
        except Exception as e:
            QMessageBox.warning(self, "写入失败", str(e))
    ...
    def _on_param_save(self):
        ...
        try:
            ...
            # 同步更新内存中的 bundle.fields 并写回 .xdat 文件
            bundle = next((b for b in self._backend.bundles
                           if b.servo_id == scs_id), None)
            if bundle is not None and row["name"] in (
                "最小角度限制", "最大角度限制", "位置偏移",
                "最大扭矩", "保护扭矩", "过载扭矩", "保护时间",
            ):
                bundle.fields[row["name"]] = value
                # 同步 min_angle/max_angle/ofs 缓存
                if row["name"] == "最小角度限制":
                    bundle.min_angle = value
                elif row["name"] == "最大角度限制":
                    bundle.max_angle = value
                elif row["name"] == "位置偏移":
                    bundle.ofs = value
                saved = self._save_xdat(scs_id)
                xdat_note = f"，已写入 {scs_id}.xdat" if saved else "，⚠ xdat 保存失败"
                ...
                xdat_note = ""
            self._status(
                f"✅ ID {scs_id} {row['name']} = {value} 已写入 EPROM{xdat_note}"
            )
        except SafetyViolation as e:
            QMessageBox.warning(self, "急停中", str(e))
        except Exception as e:
    ...
    def _on_rps_start(self):
        try:
            ...
            self.rps_log.appendPlainText(
                f"[{time.strftime('%H:%M:%S')}] 游戏启动，胜出映射: {win_map}"
            )
            self._rps_engine.start()
    ...
    def _set_controls_enabled(self, enabled):
        # 控制台 Tab
        self.apply_safety_btn.setEnabled(enabled)
        self.apply_limits_from_sim_btn.setEnabled(enabled)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="servo_console",
        description=(
            "STS3215 × 17 舵机控制台（PyQt5 GUI）。"
            "可选 --remote-tcp-port 启用远端姿态接收（与 orca_sim/live_control.py 联动）。"
        ),
        add_help=True,
    )
    parser.add_argument(
        "--remote-tcp-port",
        "--tcp-port",
        dest="remote_tcp_port",
        type=int,
        default=None,
        help="启用远端姿态接收：连接到 127.0.0.1:<port>，由 orca_sim/live_control.py 推送 17 路舵机目标。",
    )
    args, qt_argv = parser.parse_known_args(sys.argv[1:])

    app = QApplication([sys.argv[0], *qt_argv])
    app.setStyle("Fusion")
    win = ConsoleWindow(remote_tcp_port=args.remote_tcp_port)
    win.show()
    sys.exit(app.exec_())
