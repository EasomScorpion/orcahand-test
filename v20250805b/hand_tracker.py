...
SERVO_CONFIG: dict[int, dict] = {
    ...
    13: {"min": 1700, "max": 2522, "type": "thumb_big",     "finger": "thumb",  "invert": False, "name": "拇指大根部"},
    14: {"min": 1641, "max": 2156, "type": "thumb_lateral", "finger": "thumb",  "invert": False, "name": "拇指小根部"},
...
ANGLE_RANGES = {
    ...
    "thumb_big":      (41.0, 63.0),   # thumb root (tilted metacarpal axis)
    "thumb_lateral":  (25.0, 62.0),   # thumb splay in palm plane (abs)
    "thumb_mid":      (3.0, 30.0),    # thumb MCP/IP flexion
    "thumb_tip":      (2.0, 21.0),    # thumb tip (coupled)
...
class HandTracker:
    ...
    def landmarks_to_angles(self, landmarks) -> dict[int, float]:
        ...
        if thumb_ok and palm_ok:
            # Root (servo 13): metacarpal rotation about 30°-tilted axis
            tilt = math.radians(30.0)
            root_axis = _unit(math.cos(tilt) * palm_z +
                              math.sin(tilt) * (-palm_x))
            ...
            angles[13] = math.degrees(
                abs(_signed_angle_about_axis(ref_p, meta_p, root_axis)))

            # Mid (servo 16): IP flexion (2→3→4)
            angles[16] = _flexion_deg(_angle_at_rad(pts[1], pts[2], pts[3]))

            # Tip (servo 15): coupled from IP
            angles[15] = angles.get(16, 0.0) * 0.7

            # Lateral splay (servo 14): combined thumb direction in palm plane
            v_thumb = _unit(_proj(pts[2] - pts[0]) +
                            _proj(pts[3] - pts[2]))
            ref_p = _proj(palm_y)
            angles[14] = math.degrees(
                _signed_angle_about_axis(ref_p, v_thumb, palm_z))
        ...
        for sid, cfg in SERVO_CONFIG.items():
            ...
            if sid in angles:
                ...
                if jtype in ("bottom", "thumb_lateral") and sid in LATERAL_MAP:
                    ...
                    if mode == 'B':
                        # Strategy B: angle between MCP→PIP vectors.
                        ...
                        # Strategy A: signed deviation from mid_angle
                    ...
                    pos = max(cfg["min"] + 10, min(cfg["max"] - 10, pos))
                    positions[sid] = pos
